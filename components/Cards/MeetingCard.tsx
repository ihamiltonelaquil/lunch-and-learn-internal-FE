...
const MainCard: React.FC<{ meetingData: MeetingData }> = ({ meetingData }) => {
    ...
    var meetingStatus = "Happening in";
    var meetingOffset = diffDaysStart + " Days";
    ...
    if(diffSecondsStart > 0){
        if (diffDaysStart > 1) {
            ...
            meetingOffset = diffDaysStart + " Day";
        } else if (diffDaysStart < 1) {
            meetingOffset = diffHoursStart + " Hours";
            if (diffHoursStart < 1) {
                meetingOffset = diffMinutesStart + " Minutes";
                if (diffMinutesStart < 1) {
                    meetingOffset = diffSecondsStart + " Seconds";
                }
            }
        }
    }
        ...
        meetingOffset = "Today";
        if(diffDaysStart < -1){
            meetingOffset = Math.abs(diffDaysStart) + " Days Ago";
        }
    }
